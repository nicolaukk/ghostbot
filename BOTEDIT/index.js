...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'calma que ja ta saindo',
				success: 'acabou',
				error: {
					stick: 'deu erro faz tudo de novo',
					Iv: 'link errado'
				},
				only: {
					group: 'opa , isso aqui so funciona em grupo amigao',
					ownerG: 'pera la , apenas adm pode usar esses comandos',
					ownerB: 'so meu criador pode usar isso amigo',
					admin: 'MENSAGEM QUE APARECE QUANDO ALGUEM TENTA USAR UM COMANDO QUE SÓ PODE SER USADO PELO DONO DO BOT❌',
					Badmin: 'isso nao funciona se eu nao for adm'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["557996366750@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//COLOQUE AS INFORMAÇÕES SEM RETIRAR NENHUM SINAL			
				case 'info':
					...
					teks = `*ghostzin* : ${me.name}\n*ghostzin* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*TOTAL DE CONTATOS BLOQUEADOS* : ${blocked.length}\n*o bot ta on* : ${kyun(uptime)}`
...
//AQUI NAO PRECISA MEXER
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`flhou, no momento da conversão ${tipe} para o adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('deu erro , tente de novo.')
							})
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`to tirando um sonin: ${prefix}`)
					break
				/*case 'loli':
				...
				case 'yt2mp3':
					if (args.length < 1) return reply('onde esta o link?')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('o que voce esta procurando?')
				...
				case 'stalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('nome do usuário inválido')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('oque voce quer escrever')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('onde esta o link')
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('quem usou o comando') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('todos os chats limpos')
					break
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`alguem @${mentioned[0].split('@')[0]} ganhou adm`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`alguem @${mentioned[0].split('@')[0]} perdeu ADM COM SUCESSO`, mentioned, true)
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('escreva o numero corretamente')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('nao foi possível pois o número é privado' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque alguem pra ele ser expulso!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'alguem foi expulso'':\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`alguem foi expulso: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//NAO FUNCIONA MAIS		
				case 'simi':
					if (args.length < 1) return reply('J')
					teks = body.slice(5)
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('o bot ja ta on')
						welkom.push(from)
